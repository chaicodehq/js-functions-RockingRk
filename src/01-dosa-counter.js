...
export function calculateDosaOrder(type, quantity = 1, isSpicy = false) {
  ...
  const pricelist = Object.keys(basePrice);

  if (typeof type !== "string" || !pricelist.includes(type.toLowerCase()))
    return null;
  if (!Number.isInteger(quantity) || quantity <= 0) return null;

  let pricePerDosa = basePrice[type.toLowerCase()] + (isSpicy === true ? 10 : 0);
